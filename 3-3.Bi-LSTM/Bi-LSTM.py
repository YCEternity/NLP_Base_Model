...
def make_batch():
    ...
    words = sentence.split()  # 得到所有单词组成的List
...
class BiLSTM(nn.Module):
    def __init__(self):
        ...
        # n_hidden * 2是因为正向和反向两个维度进行拼接，即同时汲取正向、反向的信息
        self.W = nn.Linear(n_hidden * 2, n_class, bias=False)
        self.b = nn.Parameter(torch.ones([n_class]))

    def forward(self, X):
        # 把0、1位置进行交换
        input = X.transpose(0, 1)  # input : [n_step, batch_size, n_class] => [n_step, batch_size, n_class]
